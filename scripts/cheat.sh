...
EOF
  ...
  python3 "$(command -v virtualenv)" ve \
      || fatal Could not create virtual environment with "python3 $(command -v virtualenv) ve"

  _say_what_i_do Installing python requirements into the virtual environment
  ve/bin/pip3 install -r requirements.txt > "$LOG" \
      || {
  ...
  echo "$(ve/bin/pip3 freeze | wc -l) dependencies were successfully installed"
